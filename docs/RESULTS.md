...
| ③ 무라벨 이상탐지 | PatchCore-lite(ImageNet 특징) | **AUROC ~0.45(반전·실패)** → 무라벨 부적합(정직 negative) |
| **★최종: 5모델 앙상블+TTA+후처리+게이트** | 위 4모델 + **pseudo R2** 멤버 추가 | **🏆 0.9573** (ungated 0.9558, C3 0.878) |
...
| | 의사라벨수 | raw | +후처리(min800) | per-class |
|---|---|---|---|---|
...
| 트랙 | mean Dice | 비고 |
|---|---|---|
| 무누수 OOF(단일·일반화) | 0.9532 | 가장 정직한 일반화 수치(발표 메인) |
| fold0 단일+pseudo R2 | 0.9540 | 단일 모델 최고 |
| fold0 4모델 앙상블+TTA+후처리+게이트 | 0.9559 | |
| **fold0 5모델(+pseudo R2)+TTA+후처리+게이트** | **🏆 0.9573** | 최고치(앙상블 다양성 + 의사라벨 + 게이트) |
