...
## ③ 무라벨 이상탐지(ReconPatch/PatchCore-계열) — 정직한 negative
...
- 정직성 원칙(negative 그대로 보고). 더 끌어올리려면 full ReconPatch(contrastive+recon head, busbar처럼)+블랙보더 마스킹이 필요하나, 보조 트랙이라 여기서 종료. 코드 `src/recon_patch.py`.

## 다음 (계획)
- ② pseudo-label 2라운드(test 미라벨 → train fold만, 진행 중) 결과 반영.
